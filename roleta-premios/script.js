...
async function enviarParaPlanilha(tipo, isSorteado20 = false) {
  ...
  let url = 'https://script.google.com/macros/s/AKfycbx5nvwKMVWamLQJ374nFvBFidojsnmvygPbxjleKejWl-oZFayL_71fxinagZKzLYeL/exec';
  let body = {
    nome: respostas.nome,
...
async function mostrarResultado() {
  ...
  if (isNaN(apartamentos) || apartamentos < 26) {
    ...
    let qtd20 = 0;
    try {
      const resp = await fetch('https://script.google.com/macros/s/AKfycbzMHnLoz7W8bU1Ejqp3Oo449anZlFXMlIYmFM9p3s9YZDdbYg0PnOR_R5ezw4sCNL8/exec?tipo=20sorteados&contar=1');
    ...
    if (qtd20 < 20) {
      ...
      if ((blocos === 1 || blocos === 2) && parseInt(apartamentos) >= 26 && parseInt(banheiros) >= 3 && admin === 'sim') {
        ...
        if (sorteio === 'Edifica') {
          ...
          setTimeout(() => { confettiParticlesEdifica(); }, 300);
        } else {
