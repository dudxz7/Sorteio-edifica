...
async function mostrarResultado() {
  ...
  // Elemento de loading (cria se não existir)
  // Esconde o container principal durante o loading
  const mainContainer = document.querySelector('.container');
  if (mainContainer) mainContainer.style.visibility = 'hidden';
  let loadingDiv = document.getElementById('loading-verificacao');
  if (!loadingDiv) {
    ...
    loadingDiv.innerHTML = '<div style="font-size:2em;color:#1EA9C5;font-weight:bold;margin-bottom:16px;">Verificando disponibilidade...</div>' +
      '<div class="spinner" style="border: 6px solid #eee; border-top: 6px solid #1EA9C5; border-radius: 50%; width: 48px; height: 48px; animation: spin 1s linear infinite;"></div>' +
  ...
  if (isNaN(apartamentos) || apartamentos < 26) {
    ...
    // Mostra loading enquanto verifica na API
    // (loading já foi criado acima)
  ...
  // Remove loading e reexibe o container
  ...
  if (mainContainer) mainContainer.style.visibility = 'visible';
    ...
    if (qtd20 !== undefined && qtd20 < 20) {
      resultado = '<span style="font-size:2em;color:#FFD700;font-weight:bold;">VOCÊ É UM DOS 20 SORTEADOS!</span><br>Parabéns! Você faz parte do grupo especial.';
      classe = 'sucesso';
    ...
    if (logoDiv) logoDiv.classList.add('verdeacqua');
    document.body.classList.add('bg-verdeacqua');
    setTimeout(() => { confettiParticlesVerde(); }, 300);
  } else if (admin === 'sim') {
